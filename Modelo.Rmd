...
levels(bicicletas$season) <- c("Invierno", "Primavera", "Verano", "Otonno")
...
data.frame("fecha" = training$dteday,
           "error" = residuos_glm) %>% 
  ggplot(aes(x = fecha, y = error)) +
  geom_point(color = paleta_colores[3]) +
  labs(x = "Fecha", y = "Residuos") +
  theme_minimal()

# Se identifican los residuos mayores a 2 (en valor absoluto)
resid_grandes <- sort(abs(residuos_glm), decreasing = TRUE)[1:sum(abs(residuos_glm) > 2.5)]
...
* 27-8-2011: Se registra el acercamiento del [Huracán Irene](https://www.bbc.com/mundo/video_fotos/2011/08/110827_galeria_huracan_irene_costa_este_efectos_jg), lo cual causó un alquiler menor del esperado.
...
### ACF de residuos del glm

Posteriormente, se muestra el ACF de los residuos resultantes del modelo GLM binomial negativo.

```{r acf_residuos}
acf1(residuals(ajuste, type = "pearson"), 100, main = "")
```

En el gráfico anterior se observan autocorrelaciones significativas en los residuos de Pearson en el ACF. Esto sugiere que existe una dependencia temporal remanente que el modelo no logra explicar.
...
# Observaciones con grandes residuos
resid_grandes_tsglm <- training[which(abs(residuos_tsglm) > 2.5), ]
...
### ACF de residuos del tsglm

Se procede mostrando el ACF de los residuos de este ajuste.

```{r acf_tsglm}
# ACF de los residuos
acf1(residuos_tsglm, 100, main = "")
```
...
```{r pit_tsglm}
pit(ajuste_tsglm, main = "", col = paleta_colores[4])
```
...
(
  ajuste_tsglm <-
    interv_test(
      ajuste_tsglm,
      resid_grandes_tsglm$instant - 7,
      rep(0, nrow(resid_grandes_tsglm)),
      est_interv = TRUE
    )
)
...
Se redujo la cantidad de observaciones atípicas, de 14 a 9, en comparación al primer ajuste por tsglm. Por otro lado, parece que el rango de residuos se redujo ligeramente por abajo, pero aumentó considerablemente por arriba.

Sobre los residuos que restan en este modelo, se tienen algunas explicaciones:

* 28-8-2011: Se registra el acercamiento del [Huracán Irene](https://www.bbc.com/mundo/video_fotos/2011/08/110827_galeria_huracan_irene_costa_este_efectos_jg), lo cual causó un alquiler menor del esperado.
* 11-3-2011: El estado de Washington fue afectado, indirectamente, por el terremoto de 9.0 en la escala de Richter y posterior [tsunami](https://www-tsunami-gov.translate.goog/previous.events/?p=03-11-11_Honshu&_x_tr_sl=en&_x_tr_tl=es&_x_tr_hl=es&_x_tr_pto=tc) ocurrido en Japón. Esto parerce haber disminuido el alquiler de bicicletas.
* 30-10-2011: Una inesperada [tormenta de nieve](https://elpais.com/internacional/2011/10/30/album/1319991225_352169.html) en el norte de EEUU azotó el país en vísperas de Halloween. Esto parece haber reducido los alquileres de bicicletas.

Lo anterior indica que el cambio en el modelo mitigó algunos de los problemas, pero magnifica otros.

### ACF del tsglm considerando atípicos

Adicionalmente, el ACF de los residuos restantes.

```{r acf_tsglm_atipicos}
acf1(residuals(ajuste_tsglm, type = "pearson"), 100, main = "")
```

El ACF de los residuos de Pearson parece haber empeorado en este ajuste, pues se muestran más valores significativos que en el ajuste anterior. Aunque este sigue siendo bastante mejor que el ajuste por GLM.
...
```{r prediccion}
# Matriz de modelo
matrizm <- model.matrix(~ dteday + lag7, data = testing)
...
data.frame(
  ...
  scale_color_manual(
    name = "Series",
    values = c("Real" = paleta_colores[1], "Predicción" = paleta_colores[4])
  ) +
  scale_fill_manual(name = "IC",
                    values = c("IC predicción" = paleta_colores[6])) +
...
Además, parece que los primeros meses la predicción no difiere tanto de las observaciones. A partir de octubre empieza a tener diferencias considerables.
...
Sin embargo, a pesar del evento atípico mencionado, se observa que casi todas las predicciones difieren en más de 100 bicicletas (92.4\%). De igual forma, existe una gran cantidad de observaciones que poseen una distancia mayor a 1000 bicicletas (33.7\%).
