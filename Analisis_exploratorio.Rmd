...
Se grafica las series de tiempo de las variables coherentes con su respectiva tendencia. No se grafican ni variables categoricas ni variables redundantes como el dia de la semana.  
...
ggplot(bicicletas, aes(x = dteday, y = cnt)) +
  geom_line(color = paleta_colores[1], linewidth = 1) +
  geom_smooth(method = "lm", se = FALSE, color = paleta_colores[4], linewidth = 1.2) +
  labs(x = "Fecha", y = "Alquileres totales") +
  theme_minimal()
...
bicicletas$cnt_diff <- c(NA, diff(bicicletas$cnt))

ggplot(bicicletas, aes(x = dteday, y = cnt_diff)) +
  geom_line(color = paleta_colores[6], linewidth = 1) +
  labs(title = "Serie diferenciada del conteo de alquileres",
       x = "Fecha", y = "Δ Conteo (primera diferencia)") +
  theme_minimal()
