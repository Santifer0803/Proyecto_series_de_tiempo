...
```{r, alquileres totales por días festivos}
ggplot(bicicletas,
...
```{r, alquileres casuales por día festivo}
ggplot(bicicletas,
...
```{r, alquileres regulares por día festivo}
ggplot(bicicletas,
...
```{r, alquileres totales por día laboral}
ggplot(bicicletas,
...
```{r, Alquileres casuales por día laboral}
ggplot(bicicletas,
...
```{r, alquileres regulares por día laboral}
ggplot(bicicletas,
...
ggplot() +
  ...
  geom_smooth(data = bicicletas, aes(x = dteday, y = casual, color = "Alquileres casuales"),
  ...
  geom_smooth(data = bicicletas, aes(x = dteday, y = registered),
              method = "lm", se = FALSE, color = "black", linewidth = 1.8, linetype = "dashed") +
  geom_smooth(data = bicicletas, aes(x = dteday, y = registered, color = "Alquileres registrados"),
              method = "lm", se = FALSE, linewidth = 1.2, linetype = "dashed") +

  # Colores personalizados desde la paleta
  scale_color_manual(values = c(
    "Alquileres casuales" = paleta_colores[8],
    "Alquileres registrados" = paleta_colores[12]
  )) +
...
```{r acf_temp}
acf(bicicletas$temp, lag.max = 31, main = "")
```
...
```{r acf_windspeed}
acf(bicicletas$windspeed, lag.max = 31, main = "")
```
...
```{r acf_casual}
acf(bicicletas$casual, lag.max = 31, main = "")
```
...
```{r acf_cnt}
acf(bicicletas$cnt, lag.max = 31, main = "")
```
...
```{r acf_diff_cnt}
acf(diff(bicicletas$cnt), lag.max = 31, main = "")
```
...
data.frame(freq = cnt_ws$freq,
           coh = cnt_ws$coh) %>%
  ggplot(aes(x = freq, y = coh)) +
  geom_line(color = paleta_colores[4], linewidth = 1) +
  geom_hline(yintercept = C, color = paleta_colores[9]) +
  labs(x = "Frecuencia (diaria)",
       y = "Coherencia cuadrada") +
...
# Se grafica la coherencia cuadrada
data.frame(freq = cnt_atemp$freq,
           coh = cnt_atemp$coh) %>%
  ggplot(aes(x = freq, y = coh)) +
  geom_line(color = paleta_colores[4], linewidth = 1) +
  geom_hline(yintercept = C, color = paleta_colores[9]) +
